...
def get_master_agent_runnable():
    """
    사용자의 질문을 분석하여 가장 적합한 도구(전문가 에이전트)를 선택하거나,
    일반 대화로 처리하는 마스터 에이전트의 실행 체인을 생성합니다.
    """
    rendered_tools = render_text_description(available_tools)
    
    # CHANGED: Made the prompt stricter to ensure JSON-only output
    prompt_template = ChatPromptTemplate.from_template(
        ...
        **모든 판단과 분석은 한국어를 기준으로 합니다.**
        ...
        ---
        중요: 당신의 답변은 오직 JSON 객체여야 합니다. JSON 앞뒤로 어떠한 설명이나 추가 텍스트도 붙이지 마세요.
